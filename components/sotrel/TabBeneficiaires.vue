...
<template>
  ...
  <div v-else>
    ...
    >
      <!-- Chargement -->
      <div v-if="loadingBenef" class="flex flex--column flex--align-center gap--medium py--jumbo px--regular text--neutral-600 text--center">
        <LcLoader variant="primary" size="md" />
      </div>
        ...
        <div class="flex flex--column gap--medium p--regular bg--neutral-100 radius">
          ...
          <div class="grid grid--2-col grid--fluid gap--regular">
            <div class="flex flex--column gap--micro">
              ...
              <span class="text--body text--neutral-900 text-mono">{{ selectedBenef.NUM_BENEF || '–' }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Adresse</span>
              <span class="text--body text--neutral-900">
                {{ [selectedBenef.ADRESSE1_BENEF, selectedBenef.ADRESSE2_BENEF, selectedBenef.ADRESSE3_BENEF].filter(Boolean).join(', ') || '–' }}
                <br v-if="selectedBenef.CODE_POSTAL_BENEF || selectedBenef.VILLE_BENEF" />
                {{ [selectedBenef.CODE_POSTAL_BENEF, selectedBenef.VILLE_BENEF].filter(Boolean).join(' ') }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex--column gap--medium p--regular bg--neutral-100 radius">
          ...
          <div class="grid grid--2-col grid--fluid gap--regular">
            <div class="flex flex--column gap--micro">
              ...
              <span class="text--body text--neutral-900">{{ formatCurrency(selectedBenef.CAPITAL_BENEF) }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Destinataire</span>
              <span class="text--body text--neutral-900">{{ formatCurrency(selectedBenef.DESTINATAIRE_BENEF) }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Enfant(s)</span>
              <span class="text--body text--neutral-900">{{ formatCurrency(selectedBenef.ENFANT_BENEF) }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Date capital</span>
              <span class="text--body text--neutral-900">{{ selectedBenef.DATE_CAPITAL_BENEF ? formatDate(selectedBenef.DATE_CAPITAL_BENEF) : '–' }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Mode de paiement</span>
              <span class="text--body text--neutral-900">{{ selectedBenef.MODE_PAIE_BENEF || '–' }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex--column gap--medium p--regular bg--neutral-100 radius">
          ...
          <div class="grid grid--2-col grid--fluid gap--regular">
            <div class="flex flex--column gap--micro">
              ...
              <span class="text--body text--neutral-900">{{ selectedBenef.DOMICILIATION_BENEF || '–' }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Code banque</span>
              <span class="text--body text--neutral-900 text-mono">{{ selectedBenef.CODE_BQ_BENEF || '–' }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Guichet</span>
              <span class="text--body text--neutral-900 text-mono">{{ selectedBenef.GUICHET_BENEF || '–' }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Compte</span>
              <span class="text--body text--neutral-900 text-mono">{{ selectedBenef.COMPTE_BENEF || '–' }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Clé</span>
              <span class="text--body text--neutral-900 text-mono">{{ selectedBenef.CLE_BENEF || '–' }}</span>
            </div>
          </div>
        </div>

        <!-- ── Complément (conditionnel) ── -->
        <div v-if="selectedBenef.COMPLEMENT_BENEF" class="flex flex--column gap--medium p--regular bg--neutral-100 radius">
          ...
          <div class="grid grid--2-col grid--fluid gap--regular">
            <div class="flex flex--column gap--micro">
              ...
              <span class="text--body text--neutral-900">{{ formatCurrency(selectedBenef.COMPLEMENT_BENEF) }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Destinataire</span>
              <span class="text--body text--neutral-900">{{ formatCurrency(selectedBenef.DESTINATAIRE_COMPL_BENEF) }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Enfant(s)</span>
              <span class="text--body text--neutral-900">{{ formatCurrency(selectedBenef.ENFANT_COMPL_BENEF) }}</span>
            </div>
            <div class="flex flex--column gap--micro">
              <span class="text--caption-semibold text--neutral-600">Date complément</span>
              <span class="text--body text--neutral-900">{{ selectedBenef.DATE_COMPL_BENEF ? formatDate(selectedBenef.DATE_COMPL_BENEF) : '–' }}</span>
            </div>
          </div>
        </div>
      ...
      <div v-else class="flex flex--column flex--align-center gap--medium py--large text--neutral-600 text--center">
        <p>Aucun détail disponible pour ce bénéficiaire.</p>
      </div>

      <!-- Footer -->
      <div class="flex flex--justify-end gap--small mt--regular">
        <LcButton variant="primary" icon-left="x" @click="showModal = false">Fermer</LcButton>
      </div>
    </LcModal>
  </div>
</template>
